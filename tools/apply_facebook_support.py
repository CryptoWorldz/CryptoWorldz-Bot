...
BUILD = '2026-09-18-donate-home-v2'
...
# Keep the DonateWorldz home page simple, visual and mobile-first.
home = ROOT/'donateworldz.com/index.html'
home.write_text(f'''<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1,viewport-fit=cover"><meta name="color-scheme" content="dark"><title>DonateWorldz | Make the Difference</title><meta name="description" content="Choose one clear OneWorldz support pathway."><link rel="stylesheet" href="/style.css"><link rel="stylesheet" href="/visual-fix.css"><style>:root{{--accent:#a855f7;--accent2:#38bdf8}}.donate-home{{max-width:1080px;margin:14px auto 0}}.donate-home .big-title{{max-width:760px}}.home-intro{{max-width:680px;font-size:1.08rem}}.path-grid{{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:14px;margin-top:22px}}.path-card{{display:flex;flex-direction:column;gap:10px;min-height:190px;padding:22px;border:1px solid rgba(255,255,255,.15);border-radius:20px;background:linear-gradient(160deg,rgba(255,255,255,.07),rgba(255,255,255,.025));box-shadow:0 16px 44px rgba(0,0,0,.22);transition:transform .18s ease,border-color .18s ease,box-shadow .18s ease}}.path-card:hover,.path-card:focus-visible{{transform:translateY(-2px);border-color:var(--accent);box-shadow:0 18px 46px rgba(0,0,0,.32),0 0 28px rgba(168,85,247,.13);outline:none}}.path-card b{{font-size:clamp(1.3rem,2.2vw,1.85rem);line-height:1.12}}.path-card span{{color:#d9d2e8;line-height:1.45}}.path-card .open{{margin-top:auto;padding-top:8px;color:var(--accent2);font-weight:950;font-size:.82rem;letter-spacing:.06em}}.path-card.slice{{border-color:rgba(168,85,247,.65);background:linear-gradient(145deg,rgba(91,33,124,.48),rgba(10,6,18,.94))}}@media(max-width:760px){{.donate-home{{margin-top:8px}}.home-intro{{font-size:1rem}}.path-grid{{grid-template-columns:1fr;gap:10px;margin-top:16px}}.path-card{{min-height:0;padding:18px;border-radius:16px}}.path-card b{{font-size:1.28rem}}}}</style></head><body data-oneworldz-build="{BUILD}"><nav class="nav"><a class="brand" href="/">DonateWorldz</a><a href="https://oneworldz.com">OneWorldz</a></nav><main class="shell"><section class="section donate-home"><p class="eyebrow">DonateWorldz</p><h1 class="big-title">Make the Difference</h1><p class="home-intro">Four clear ways to help. Tap a pathway.</p><div class="path-grid"><a class="path-card" href="/davis-family/"><b>Davis Family</b><span>Direct family support.</span><span class="open">OPEN →</span></a><a class="path-card slice" href="/slice-of-hope-australia/"><b>🍕 Slice of Hope Australia</b><span>Community meals starting in Dubbo, with a vision to grow across Australia.</span><span class="open">OPEN →</span></a><a class="path-card" href="/community-impact/"><b>Community Impact</b><span>Support community causes and people helping people.</span><span class="open">OPEN →</span></a><a class="path-card" href="/jayjay-support/"><b>Support JayJayTeamDev</b><span>Support the work behind OneWorldz.</span><span class="open">OPEN →</span></a></div></section></main><footer class="footer"><strong>Created with the Vision</strong><br>Make the Difference • OneWorldz 🌏 One Vision</footer></body></html>''',encoding='utf-8')
