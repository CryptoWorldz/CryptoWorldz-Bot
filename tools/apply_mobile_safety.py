...
CSS=r'''html,body{max-width:100%!important;overflow-x:hidden!important}body{min-width:0!important}img,video,svg,canvas{max-width:100%!important;height:auto!important}.nav{max-width:100%!important;flex-wrap:wrap!important}.nav a{min-width:0!important;max-width:100%!important;overflow-wrap:anywhere!important;text-align:center!important}.shell,.hero,.section,.grid,.grid2,.portal,.hero-grid,.hero-list,.system-grid,.support-grid,.fb-grid,.mission,.tokens,.token-grid,.mission-band,.split,.hero-profile{min-width:0!important;max-width:100%!important}.hero img,.hero-art,.feature-img,.card img,.hero-card img,.visual-card img,.world-card img,.hope img,.hero-profile img{width:100%!important;height:auto!important;max-height:none!important;object-fit:contain!important;object-position:center!important}.btn,.chip,.hero-card,.card,.world-card,.info-card,.token-card,.visual-card,.support-card{min-width:0!important;max-width:100%!important;overflow-wrap:anywhere!important}.ow-home-button{position:fixed;z-index:10000;top:10px;left:10px;display:inline-flex;align-items:center;justify-content:center;min-height:46px;padding:0 13px;border:1px solid rgba(255,255,255,.28);border-radius:14px;background:rgba(5,4,10,.94);color:#fff!important;text-decoration:none!important;font:900 14px/1 system-ui,-apple-system,"Segoe UI",sans-serif;box-shadow:0 10px 30px rgba(0,0,0,.42)}@media(max-width:720px){.nav{position:relative!important;display:grid!important;grid-template-columns:repeat(2,minmax(0,1fr))!important;gap:8px!important;padding:8px!important}.nav a{width:100%!important;padding:10px 8px!important;white-space:normal!important}.shell{width:100%!important;padding:10px!important}.hero-grid,.split,.hero-profile{grid-template-columns:1fr!important}.grid,.grid2,.portal,.hero-list,.system-grid,.support-grid,.fb-grid,.mission,.tokens,.token-grid{grid-template-columns:1fr!important}.btns,.actions{display:grid!important;grid-template-columns:1fr!important;width:100%!important}.btn{width:100%!important}.ow-home-button{width:46px;height:46px;padding:0;border-radius:13px;font-size:0}.ow-home-button:before{content:'⌂';font-size:24px}}@media(max-width:360px){.nav{grid-template-columns:1fr!important}}'''
...
for domain in DOMAINS:
    for path in (ROOT/domain).rglob('*.html'):
        ...
        if 'class="ow-home-button"' not in text:
            ...
            if count!=1:
                raise SystemExit(f'NO_BODY={path.relative_to(ROOT)}')
        path.write_text(text,encoding='utf-8')
        html_files.append(path)

heroes=[
 ('Just Knate','just-knate','just-knate.webp','Direct help and dignity'),
 ('Victor — The Good Boss','victor-good-boss','victor-good-boss.webp','Recovery, hope and second chances'),
 ('Sam Weidenhofer','sam-weidenhofer','sam-weidenhofer.webp','Everyday community kindness'),
 ('Dylan Thiry','dylan-thiry','dylan-thiry.webp','Building hope and stronger futures'),
 ('Bi Phakathi','bi-phakathi','bi-phakathi.webp','Compassion in action'),
 ('MDMotivator','mdmotivator','mdmotivator.webp','Global kindness and encouragement'),
 ('Bob — The Giving Roofer','bob-roofer','heroes-world.webp','Practical community generosity'),
]
hero_dir=ROOT/'oneworldz.com/assets/heroes'
for _,_,asset,_ in heroes:
    p=hero_dir/asset
    if not p.is_file() or p.stat().st_size<10000:
        raise SystemExit(f'HERO_ASSET_MISSING={asset}')
