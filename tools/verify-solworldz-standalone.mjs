...
assert.match(html,/solworldz-hero-desktop\.png/,'Desktop approved hero missing');
assert.match(html,/solworldz-hero-mobile\.png/,'Mobile approved hero missing');
assert.match(html,/solworldz-approved-atlas\.png/,'Approved SolWorldz atlas missing');
...
for(const name of ['solworldz-hero-desktop.png','solworldz-hero-mobile.png','solworldz-approved-atlas.png']){assert.ok(fs.existsSync(media(name)),`Approved media missing: ${name}`);assert.ok(fs.statSync(media(name)).size>50000,`Approved media unexpectedly small: ${name}`)}
...
console.log('SolWorldz production gate passed: approved PNG hero/atlas, current pipeline and Robin Hood Chain classification verified.');
