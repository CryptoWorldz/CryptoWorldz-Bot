...
async function getText(cdp) {
  const r = await cdp.send('Runtime.evaluate', { expression: `({title:document.title||'',body:(document.body?.innerText||'').slice(0,1200)})`, returnByValue: true });
...
async function clearAndNavigate(cdp, url, responses, failures) {
  ...
  const challenge = `${text.title}\n${text.body}`.toLowerCase().includes('checking your browser before accessing') || `${text.title}\n${text.body}`.toLowerCase().includes('please wait for up to 5 seconds');
  if (challenge) {
...
const chrome = spawn(chromePath(), [
  ...
  `--user-data-dir=/tmp/worldz-browser-${process.pid}`,'--hide-scrollbars','about:blank'
