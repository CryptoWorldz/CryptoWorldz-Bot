...
prove_one() {
  ...
  local public_ok=0 attempt curl_rc proof_url mode
  for attempt in 1 2 3 4; do
    ...
    if (( attempt <= 2 )); then
      ...
      mode='canonical'
      proof_url="$LIVE_URL/$rel"
    fi
    set +e
    curl -L -fsS -D "$headers" \
      -A 'Mozilla/5.0 WorldzProductionProof/2.2' \
    ...
    if [[ "$curl_rc" -eq 0 && -s "$public" ]]; then
      public_ok=1
      echo "Public CDN media fetch passed using $mode URL on attempt $attempt for $rel"
      break
    fi
    echo "::warning::Public CDN media fetch attempt $attempt ($mode) failed for $rel (curl exit $curl_rc). Exact FTP-origin bytes already passed."
    sleep $((attempt * 2))
  done
  if [[ "$public_ok" != '1' ]]; then
    echo "::error::Public CDN media could not be fetched by cache-busted or canonical URL after 4 bounded attempts for $rel" >&2
    return 1
  fi
