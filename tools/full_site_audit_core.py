...
def ecosystem_urls() -> list[str]:
    ...
    assert len(items) == len(set(items)) and 18 <= len(items) <= 80, f'expected mission public set (18-80 URLs), got {len(items)} total / {len(set(items))} unique'
    return items
...
def static_audit() -> None:
    ...
    for url in urls:
        ...
        chain_gateway = 'data-build="2026-09-24-chain-launch-gateway-v1"' in text
        if chain_gateway:
            if '<style>' not in text:
                errors.append(f'MISSING CHAIN GATEWAY INLINE STYLE {f.relative_to(ROOT)}')
            if '/style.css' in text:
                errors.append(f'LEGACY CHAIN STYLE LINK {f.relative_to(ROOT)}')
        else:
        ...
        if not chain_gateway and 'data-oneworldz-build=' not in text:
...
def live_audit() -> None:
    ...
    def check(item):
        ...
        if ext in IMAGE_EXTS:
            ...
            chain_gateway = 'data-build="2026-09-24-chain-launch-gateway-v1"' in text
            if chain_gateway:
                if '<style>' not in text or '/style.css' in text:
                    return f'LIVE CHAIN GATEWAY VISUAL CONTRACT FAILED {url}'
            else:
                if (
