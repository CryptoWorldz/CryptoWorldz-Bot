...
BUILD='2026-09-05-architecture-v3'

# DonateWorldz: three active pathways plus one reserved future support slot.
# The reserved Stripe destination remains untouched and is not publicly linked until reassigned.
donate=ROOT/'donateworldz.com'
nav='''<nav class="nav"><a class="brand" href="/">DonateWorldz</a><a href="https://oneworldz.com">OneWorldz</a><a href="/davis-family/">Davis Family</a><a href="/community-impact/">Community Impact</a><a href="/jayjay-support/">JayJayTeamDev</a></nav>'''
cards=[
 ('Future Support Partner','Reserved for a future person, family or orphanage. No donations accepted through this slot until a new recipient is approved.',None,None),
...
for name,desc,href,img in cards:
    ...
    if href:
        ...
        parts.append(f'<article class="card donate-path reserved-support">{visual}<b>{escape(name)}</b><span>{escape(desc)}</span><i>RESERVED • NOT ACTIVE</i></article>')
body=f'''<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1,viewport-fit=cover"><meta name="color-scheme" content="dark"><title>DonateWorldz | Direct Support Pathways</title><link rel="stylesheet" href="/visual-fix.css"><style>.donate-path{{min-height:190px;justify-content:center}}.donate-path img{{max-height:360px;object-fit:contain}}.donate-path b{{font-size:1.35rem}}.reserved-support{{opacity:.78;border-style:dashed}}</style></head><body data-oneworldz-build="{BUILD}">{nav}<main class="shell"><section class="section"><p class="ey">Give clearly • Support directly</p><h1>DonateWorldz</h1><p>Three active, separated support pathways plus one future support slot held in reserve. Payment details stay on Stripe.</p><div class="grid2">{''.join(parts)}</div></section></main><div class="foot">OneWorldz 🌐 One Vision • Helping the People Who Help People</div></body></html>'''
...
for href in ['/davis-family/','/community-impact/','/jayjay-support/']:
    assert donate_text.count(f'href="{href}"') >= 1, href
assert 'Future Support Partner' in donate_text and 'RESERVED • NOT ACTIVE' in donate_text
for retired in ['reagan','action spreads smiles','action creates smiles']:
    assert retired not in donate_text.lower(), retired
...
print('ARCHITECTURE_FINISH=PASS active_donate_paths=3 reserved_future_slot=1 retired_uganda=0 chain_worldz=8 unique_themes=8')
