...
BUILD = '2026-09-14-davis-family-final-v3'

main_b64 = ''.join((CHUNKS / f'main.{i:02d}').read_text(encoding='utf-8').strip() for i in range(5))
profile_b64 = (CHUNKS / 'profile.00').read_text(encoding='utf-8').strip()
...
assert len(main_bytes) > 20000 and len(profile_bytes) > 4000
...
<title>Davis Family | DonateWorldz</title><meta name="description" content="Support the Davis Family through kindness, compassion and hope.">
<meta property="og:title" content="Davis Family | DonateWorldz"><meta property="og:description" content="One kind act changes everything. Support the Davis Family through DonateWorldz."><meta property="og:image" content="https://donateworldz.com{MAIN}">
<meta name="twitter:card" content="summary_large_image"><meta name="twitter:image" content="https://donateworldz.com{MAIN}">
<link rel="stylesheet" href="/style.css"><link rel="stylesheet" href="/visual-fix.css">
...
<nav class="nav davis-nav"><a class="davis-home" href="/" aria-label="Return to DonateWorldz home"><img src="{PROFILE}" alt="DonateWorldz" width="128" height="128"></a><a class="davis-nav-title" href="/">DonateWorldz</a><a href="https://oneworldz.com">OneWorldz</a></nav>
<main class="shell davis-shell"><section class="section davis-hero"><img class="davis-main-image" src="{MAIN}" alt="One Kind Act Changes Everything — Donate, Share, Hope" width="600" height="624" loading="eager" decoding="async" fetchpriority="high"><p class="davis-kicker">Direct support pathway</p><h1 class="davis-title">Davis Family</h1></section>
<section class="section"><div class="davis-story">{copy}</div><div class="davis-actions"><a class="btn" href="{STRIPE}" target="_blank" rel="noopener noreferrer">Donate securely with Stripe</a><a class="btn secondary" href="{FACEBOOK}" target="_blank" rel="noopener noreferrer">Open Facebook</a><a class="btn secondary" href="/">All Donation Pages</a></div><p class="davis-note">Payment is completed on Stripe. No card details, bank credentials or Stripe secrets are stored on this website.</p></section></main>
<footer class="footer"><strong>Created with the Vision</strong><br>When Someone say’s You can’t Change the World 🌐 just say “Why can’t I?”<br>Make the Difference • OneWorldz 🌏 One Vision</footer></body></html>'''
...
print(f'DAVIS_PAGE=PASS main_bytes={len(main_bytes)} profile_bytes={len(profile_bytes)} image=1 profile_home=1 copy=1 stripe=1 facebook=1 mobile=1')
