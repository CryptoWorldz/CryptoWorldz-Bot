...
for (const row of rows) {
  ...
  const nameVisible = !expectedName || expected?.restricted || text.includes(expectedName.toLowerCase());
  ...
  results.push({
    ...
    expected_name_visible: nameVisible,
    status,
    final_url: finalUrl,
    canonical,
    title,
  ...
  console.log(`FACEBOOK_${String(row.display_order).padStart(2, '0')}=${pass ? 'PASS' : 'FAIL'} status=${status} id=${shareId} expected=${JSON.stringify(expectedName)} final=${JSON.stringify(finalUrl)}`);
}
await browser.close();
...
if (failed.length) {
  console.error('FAILED_COMMUNITY_FACEBOOK=' + JSON.stringify(failed.map((row) => ({ order: row.order, url: row.url, status: row.status, final_url: row.final_url, title: row.title, explicit_error: row.explicit_error, navigation_error: row.navigation_error }))));
  process.exit(1);
}
