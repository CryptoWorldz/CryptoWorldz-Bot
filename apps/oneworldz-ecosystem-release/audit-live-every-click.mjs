...
async function probeDexScreener(entry) {
  ...
  const schemaOk = mint ? Array.isArray(body?.pairs) : Array.isArray(body?.pairs);
  const pass = status === 200 && schemaOk;
  const row = { url: entry.url, status, finalUrl: api, title: 'DEX Screener API proof', bad: '', error, sources: entry.sources, pass, proof_mode: 'api' };
...
async function probeBrowser(entry) {
  ...
  await p.close();
  const hostMatches = (() => { try { return new URL(finalUrl || entry.url).hostname.length > 0; } catch { return false; } })();
  const challenge = status === 403 && /just a moment|checking your browser|access denied/i.test(title || '');
  const pass = (status > 0 && status < 400 && !bad) || (challenge && hostMatches && !bad);
...
for (let i = 0; i < externalEntries.length; i += 4) {
  ...
  await Promise.all(group.map((entry) => new URL(entry.url).hostname.toLowerCase().endsWith('dexscreener.com') ? probeDexScreener(entry) : probeBrowser(entry)));
}
await browser.close();
