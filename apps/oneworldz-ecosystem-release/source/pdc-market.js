(() => {
  ...
  function render(card, pair) {
    ...
    if (!market || !dexLink) return;

    if (!pair) {
      market.dataset.state = 'none';
      market.innerHTML = '<small>DEX Screener</small><strong>No active pair found</strong><span>Legacy record remains independently verifiable on Solscan.</span>';
      dexLink.hidden = true;
      return;
    }
  ...
  async function load() {
    try {
      ...
      if (status) { status.textContent = 'DEX Screener market view loaded'; status.dataset.state = 'ready'; }
    } catch {
      for (const card of cards) render(card, null);
      if (status) { status.textContent = 'Live market data unavailable — Solscan history remains available'; status.dataset.state = 'error'; }
    }
  }
