(() => {
  ...
  function installStyles() {
    ...
    style.textContent = `
      .ow-hero-art{overflow:hidden;border-radius:24px;background:#09030f}
      .ow-hero-art img{width:100%!important;height:auto!important;max-height:none!important;object-fit:contain!important;object-position:center!important;aspect-ratio:auto!important}
      ...
      @media (max-width:1120px){
        .ow-hero-network{grid-template-columns:repeat(2,minmax(0,1fr))}
        .ow-smiles-gallery{grid-template-columns:1fr 1fr}
        .ow-smiles-gallery .ow-smiles-main{grid-column:1/-1;grid-row:auto}
      }
      @media (max-width:760px){
        .ow-core-story-image{max-height:none}.ow-core-worldz-art{max-height:none}
        .ow-feature-shell{padding:24px 16px;border-radius:22px}
        .ow-hero-network{display:flex;overflow-x:auto;scroll-snap-type:x mandatory;padding-bottom:8px;gap:12px}
        .ow-hero-card{flex:0 0 min(82vw,330px);scroll-snap-align:start}
        .ow-smiles-gallery{grid-template-columns:1fr}
        .ow-destination-frame img,.ow-smiles-gallery img,.ow-hero-art img{width:100%!important;height:auto!important;object-fit:contain!important}
        .ow-impact-gallery.ow-core-impact-gallery>img{width:100%;max-height:none}
      }
    `;
  ...
  function buildHeroNetwork() {
    ...
    section.innerHTML = `
      <header class="ow-feature-head">
        <p class="eyebrow">ONEWORLDZ HEROES</p>
        <h2>Helping the People Who Help People.</h2>
        <p>OneWorldz shines a light on people already doing the work — feeding, supporting, rebuilding, encouraging and creating real change.</p>
      </header>
  ...
  function buildDestinations() {
    ...
    const heroNetwork = document.querySelector('#oneworldz-heroes');
    ...
    section.innerHTML = `
      <header class="ow-feature-head">
        <p class="eyebrow">DESTINATIONS OF HOPE</p>
        <h2>Everywhere Can Be Somewhere We Help.</h2>
        <p>From iconic world cities to communities far from the spotlight, kindness has no border.</p>
      </header>
      <div class="ow-destination-frame"><img src="${ASSETS.worldz}" alt="OneWorldz global destinations and connected communities" loading="lazy" decoding="async"></div>
      <div class="ow-destination-tags">${DESTINATIONS.map(place => `<span>${place}</span>`).join('')}</div>`;
    heroNetwork.after(section);
  }
  ...
  function buildSmilesGallery() {
    ...
    section.innerHTML = `
      <header class="ow-feature-head">
        ...
        <h2>Food. Shelter. Water. Education. Medical Care. Community.</h2>
        <p>The Action Spreads Smiles feature is kept together as its own humanitarian story inside OneWorldz, with Reagan and the children clearly identified.</p>
      </header>
      <div class="ow-smiles-gallery">
        <figure class="ow-smiles-main"><img src="${ASSETS.reagan}" alt="Reagan with children supported through Action Spreads Smiles in Uganda" loading="lazy" decoding="async"><figcaption>Reagan and the children — the main Action Spreads Smiles feature.</figcaption></figure>
        <figure><img src="${ASSETS.kindness}" alt="Action Spreads Smiles kindness artwork featuring children" loading="lazy" decoding="async"><figcaption>Kindness changes everything.</figcaption></figure>
        <figure><img src="${ASSETS.kidsBunk}" alt="Joyful children in the Action Spreads Smiles visual story" loading="lazy" decoding="async"><figcaption>Children, safety and brighter futures.</figcaption></figure>
      </div>`;
  ...
  function apply() {
    ...
    if (!worldz.querySelector('[data-core-art="worldz"]')) {
      const art = image(ASSETS.worldz, 'OneWorldz and CryptoWorldz connected Worldz directory artwork', 'ow-core-worldz-art');
    ...
    if (impactImage) {
      ...
      impactImage.alt = 'OneWorldz humanitarian artwork showing joyful children and practical support';
