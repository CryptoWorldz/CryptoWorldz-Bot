...
const IMPACT=config.basedBidBoardUrl||'https://www.based.bid/b/ImpactBased';
const IMPACT_PAGE='https://oneworldz.com/worldz/impactbased';
...
function worldGrid(){return [['CryptoWorldz','https://cryptoworldz.xyz'],['SolWorldz','https://solworldz.xyz'],['Purple Diamond Crew','https://purplediamondcrew.com'],['ImpactBased',IMPACT_PAGE],['EthWorldz','https://ethworldz.xyz'],['BaseWorldz','https://baseworldz.xyz'],['BNBWorldz','https://bnbworldz.xyz'],['XRPWorldz','https://xrpworldz.xyz'],['SuiWorldz','https://suiworldz.xyz'],['HyperWorldz','https://hyperworldz.xyz'],['RobinWorldz','https://robinworldz.xyz'],['BitcoinWorldz','https://bitcoinworldz.xyz'],['HodlerWorldz','https://hodlerworldz.xyz']].map(([n,u])=>`<a href="${u}" target="_blank" rel="noopener noreferrer">Visit ${n}</a>`).join('');}
...
<section class="wx-panel"><span class="wx-kicker">COMMUNITY PARTICIPATION</span><h2>People join the structure.</h2><p>OneWorldz provides the gateway. Purple Diamond Crew provides an action pathway. CryptoWorldz provides the crypto and technology hub. ImpactBased provides the purpose-led launch pathway. The individual Worldz give specialist communities their own home while remaining connected to the larger mission.</p><div class="wx-actions">${btn('https://purplediamondcrew.com','Take Action with Purple Diamond Crew','wx-btn green')}${btn('https://cryptoworldz.xyz','Explore CryptoWorldz')}${btn(IMPACT_PAGE,'Visit ImpactBased','wx-btn gold')}</div></section>

<section id="action" class="wx-panel"><span class="wx-kicker">HUMANITARIAN ACTION STAYS CENTRAL</span><h2>Donation pages remain part of OneWorldz.</h2><p>Food, water, shelter, education, medical support, dignity and hope remain visible at the master gateway. Reagan Kauja and Action Spreads Smile's Orphanage remain a real example of the structure being used for practical support rather than being separated from the wider vision.</p><div class="wx-actions"><a class="wx-btn green" href="/donate/">Open Help & Donation Directory</a><a class="wx-btn" href="/reagan-kauja/">Reagan Kauja • Action Spreads Smile's Orphanage</a></div></section>

<section class="wx-panel wx-feature"><div class="wx-hope-chest-media" style="display:flex;align-items:center;justify-content:center;min-width:0"><img src="${CHEST}" width="256" height="320" style="display:block;width:min(100%,256px);height:auto;min-height:0;aspect-ratio:4/5;object-fit:cover;border-radius:18px" alt="The OneWorldz Hope Chest"></div><div class="wx-copy"><span class="wx-kicker">STORIES • LEGACY • SECOND CHANCES</span><h2>The Hope Chest</h2><p>Useful work, lessons and community history are preserved without confusing old branding or legacy records with current production information.</p>${btn('https://purplediamondcrew.com','Visit Purple Diamond Crew')}</div></section>
...
<section class="wx-panel"><span class="wx-kicker">PURPOSE-LED LAUNCHES</span><h2>ImpactBased</h2><p>Purpose-driven projects can be introduced through ImpactBased while live-token status remains tied to verified public records.</p><div class="wx-actions">${btn(IMPACT_PAGE,'Visit ImpactBased','wx-btn green')}${btn(IMPACT,'Open Based.bid Board','wx-btn gold')}</div></section>
