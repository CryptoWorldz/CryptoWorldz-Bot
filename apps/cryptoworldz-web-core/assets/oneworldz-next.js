...
const CHEST='./assets/images/website-core/purple-diamond-crew/hope-chest-1927-approved.jpg';
...
nav.innerHTML=[['Home','#home'],['Vision','#vision'],['Framework','#framework'],['Action','#action'],['Help & Donate','/donate/'],['Reagan Kauja','/reagan-kauja/'],['CryptoWorldz','https://cryptoworldz.xyz'],['Purple Diamond Crew','https://purplediamondcrew.com']].map(([l,u])=>`<a href="${u}"${u.startsWith('http')?' target="_blank" rel="noopener noreferrer"':''}>${l}</a>`).join('');

app.innerHTML=`<div class="wx-shell">
<section id="home" class="wx-hero"><img class="wx-hero-bg" src="${HERO}" alt="OneWorldz One Vision master artwork"><div class="wx-hero-copy"><span class="wx-kicker">OneWorldz 🌏 One Vision</span><h1>OneWorldz</h1><p>The master gateway for one connected vision: people, planet, technology and leadership working together to turn ideas into measurable action.</p><div class="wx-actions"><a class="wx-btn green" href="/donate/">Help & Donate</a><a class="wx-btn" href="/reagan-kauja/">Meet Reagan Kauja</a>${btn('https://purplediamondcrew.com','Purple Diamond Crew','wx-btn gold')}${btn('https://cryptoworldz.xyz','Enter CryptoWorldz')}</div></div></section>
