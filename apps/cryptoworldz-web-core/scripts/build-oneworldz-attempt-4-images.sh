...
render() {
  ...
  "$IM" "$src" -auto-orient -resize "${width}x${height}^" -gravity "$gravity" -extent "${width}x${height}" \
    -strip -quality 88 -define webp:method=6 "$out"
  test -s "$out"
}

# Same approved concept, independently fitted for each viewport. No source is reused for a different content slot.
...
render "$SRC/desktop/oneworldz/hope-chest.png" 960 1200 "$OUT/desktop/hope-chest.webp" center
render "$SRC/mobile/hope-chest.webp" 1080 1350 "$OUT/mobile/hope-chest.webp" center
