...
test("Telegram registration replaces website and adds both new commands", async () => {
  const listeners = [];
  const removed = [];
  const messages = [];
  const bot = {
  ...
  assert.deepEqual(WEBSITE_COMMANDS.map((item) => item.command), ["websites", "solworldz"]);
  ...
  assert.ok(website && websites && solworldz);
