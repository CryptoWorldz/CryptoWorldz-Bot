...
async function send(ixs,label,s){
 ...
 if(fee===null||await conn.getBalance(owner,'confirmed')<fee)stop(label+' network fee unavailable or balance too low.');
 const sim=await rpc('simulateTransaction',[Buffer.from(raw).toString('base64'),{encoding:'base64',sigVerify:false,replaceRecentBlockhash:true,commitment:'confirmed'}]);
 if(sim?.value?.err)stop(label+' dry-run failed: '+JSON.stringify(sim.value.err));
 const message=tx.compileMessage().serialize().toString('hex');status(label+' simulated. Review the exact wallet transaction.');
 let signed;
