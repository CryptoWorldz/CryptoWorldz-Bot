...
const WORLDZ_ORIGIN='https://cryptoworldz.xyz';
...
function JupiterMobileBridge(){
  const {jupiterAdapter}=useWrappedReownAdapter({
    appKitOptions:{
      metadata:{
        name:'CryptoWorldz REVIVE',
        description:'REVIVE $RVIV mainnet launch',
        url:WORLDZ_ORIGIN,
        icons:[]
      },
...
export async function getJupiterMobileAdapter(){
  ...
  const timeout=new Promise((_,reject)=>setTimeout(()=>reject(new Error('Jupiter Mobile bridge did not initialise. Reload the REVIVE launch page and try again.')),15000));
