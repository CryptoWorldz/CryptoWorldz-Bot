function estimatePostCost(platform, hasLink, costModel = {}) {
  const platformModel = costModel?.[platform] || {};
  const key = hasLink ? "link" : "text";
  const cost = Number(platformModel[key] ?? platformModel.text ?? 0);
...
export function createGraceWorker({
  ...
  if (!repository || !graceX) throw new Error("Grace worker dependencies are incomplete.");
  let timer = null;
  let running = false;
  ...
  async function runOnce() {
    ...
    try {
      ...
      for (const target of targets) {
        ...
        const estimatedCost = estimatePostCost(
          target.platform,
          hasLink,
          settings.cost_model || {},
        );
        try {
          if (target.platform !== "x") {
            const error = new Error(
              `The clean Grace runtime does not yet have an approved ${target.platform} publisher.`,
            );
            error.code = "PLATFORM_NOT_ENABLED";
          ...
          const published = await graceX.publishText(
            target.account_id,
            target.caption,
            target.link_url,
          );
          ...
          await repository.markTargetFailed(target.target_id, error.message, {
  ...
  function start() {
    ...
    timer = setInterval(() => {
      runOnce().catch((error) =>
        logger.error("Grace worker cycle failed", {
