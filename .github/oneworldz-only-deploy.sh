...
const target = productionTargets.find((item) => item.key === 'oneworldz');
if (!target) throw new Error('OneWorldz production target missing');
...
release_root="$GITHUB_WORKSPACE/apps/oneworldz-ecosystem-release/dist/ecosystem/oneworldz"
...
proof_root="$RUNNER_TEMP/oneworldz-only"
...
# Prove the authenticated Hostinger folder is the folder currently serving OneWorldz.
...
curl --fail --silent --show-error --location --connect-timeout 15 --max-time 45 \
  ...
  "https://$domain/?oneworldz_prewrite=$GITHUB_RUN_ID" -o "$public_before"
cmp -s "$remote_before" "$public_before"
echo "ONEWORLDZ_DESTINATION_PROVED $domain -> $transport"

# Back up every file this release is about to overwrite. Missing old files are allowed.
...
rollback() {
  echo "::warning::Rolling back OneWorldz only. No other Hostinger destination is touched."
  {
    echo 'set cmd:fail-exit false'
    echo 'set net:max-retries 2'
    echo 'set net:timeout 30'
...
# Upload non-index files first; publish index.html last so the homepage switches atomically.
{
...
# Authenticated read-back must byte-match every file in the candidate.
...
# Public edge must serve the exact candidate homepage and release manifest.
...
for attempt in $(seq 1 12); do
  ...
  if curl --fail --silent --show-error --location --connect-timeout 15 --max-time 45 \
      -H 'Cache-Control: no-cache, no-store, max-age=0' -H 'Pragma: no-cache' \
      "https://$domain/?oneworldz_release=$GITHUB_RUN_ID-$attempt" -o "$public_index" \
    && curl --fail --silent --show-error --location --connect-timeout 15 --max-time 45 \
      -H 'Cache-Control: no-cache, no-store, max-age=0' -H 'Pragma: no-cache' \
      "https://$domain/release-manifest.json?oneworldz_release=$GITHUB_RUN_ID-$attempt" -o "$public_manifest" \
...
printf 'target\tdomain\tstatus\n%s\t%s\t%s\n' "$key" "$domain" 'DEPLOYED_AND_BYTE_PROVED' > deployment-proof/oneworldz-only-status.tsv
cp "$files" deployment-proof/oneworldz-only-files.txt
printf 'run_id=%s\ncommit=%s\ndomain=%s\ntransport=%s\nexpected_title=%s\n' "$GITHUB_RUN_ID" "$GITHUB_SHA" "$domain" "$transport" "$title" > deployment-proof/oneworldz-only-proof.txt

echo "ONEWORLDZ_ONLY_HOSTINGER_DEPLOYMENT=PASS domain=$domain files=$(wc -l < "$files" | tr -d ' ')"
trap - EXIT
