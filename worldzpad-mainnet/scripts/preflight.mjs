...
A(candidate.status==='PREPARED_NOT_SIGNED','status must remain PREPARED_NOT_SIGNED');
A(candidate.launchAuthorized===false,'launchAuthorized must remain false');
A(candidate.executionEnabled===false,'executionEnabled must remain false');
...
A(candidate.treasury.currentThreshold==='2-of-3','threshold declaration drift');
...
const report={
  ...
  note:'Current canonical WLDZ preparation gate. This file does not broadcast, sign, authorize, or execute a launch.',
};
fs.mkdirSync(path.join(root,'artifacts'),{recursive:true});
fs.writeFileSync(path.join(root,'artifacts','wldz-mainnet-preparation-report.json'),JSON.stringify(report,null,2)+'\n');
console.log('WLDZ_MAINNET_PREPARATION=PASS execution_enabled=0 launch_authorized=0');
