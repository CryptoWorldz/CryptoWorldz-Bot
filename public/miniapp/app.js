...
function render() {
  ...
  byId("governance-list").innerHTML = data.governance.length ? data.governance.map((item) => {
    ...
    return `<article class="panel proposal"><small>PROPOSAL #${escapeHtml(item.id)} • ${escapeHtml(item.status)}</small><h3>${escapeHtml(item.title)}</h3><p>${escapeHtml(item.description)}</p><div class="governance-options">${options.map((option, index) => `<button class="button governance-vote${String(item.selected_option) === String(index + 1) ? " selected" : ""}" type="button" data-proposal-id="${escapeHtml(item.id)}" data-option="${index + 1}" ${item.selected_option ? "disabled" : ""}><span>${index + 1}. ${escapeHtml(option)}</span><strong>${Number(item.vote_counts && item.vote_counts[String(index + 1)]) || 0} Votes</strong></button>`).join("")}</div><p><strong>Total Votes: ${Number(item.total_votes) || 0}</strong>${item.selected_option ? `<br>✅ Your Vote: Option ${escapeHtml(item.selected_option)}` : ""}</p><small>One Legend • One Vote. Legend Points are not deducted.</small></article>`;
  }).join("") : empty("Community Governance is being prepared. No financial voting is active.");
