...
function render() {
  ...
  byId("fullbuild-home-card").innerHTML = `<article class="panel"><p class="eyebrow">🌐 WORLDZFULLBUILD™</p><h3>The canonical master architecture for the complete Worldz technology build.</h3><p>WorldzFullScope™ is required inside FullBuild, alongside the existing Memory & Storage, wallet, distribution, vesting and proof standards.</p><div class="form-row"><button class="button" type="button" data-open="fullbuild">Open FullBuild</button><button class="button secondary" type="button" data-open="fullscope">Open FullScope</button></div></article>`;
  byId("fullscope-home-card").innerHTML = `<article class="panel"><p class="eyebrow">🌐 ZED LED • WORLDZFULLSCOPE™</p><h3>One command layer across every supported WorldzLaunchPad chain.</h3><p>Watch tokens, locks, vesting and proof — with two deliberately separate voting systems.</p><div class="form-row"><button class="button" type="button" data-open="fullscope">Open FullScope</button><button class="button secondary" type="button" data-open="worldz-votes">🗳️ Popularity Votes</button></div><button class="button secondary" type="button" data-open="governance">🏛️ DAO Governance</button></article>`;
  ...
  byId("governance-list").innerHTML = data.governance.length ? data.governance.map((item) => {
    const options = Array.isArray(item.options) ? item.options : [];
    return `<article class="panel proposal"><small>WORLDZGOVERN™ PROPOSAL #${escapeHtml(item.id)} • ${escapeHtml(item.status)}</small><h3>${escapeHtml(item.title)}</h3><p>${escapeHtml(item.description)}</p><div class="governance-options">${options.map((option, index) => `<button class="button governance-vote${String(item.selected_option) === String(index + 1) ? " selected" : ""}" type="button" data-proposal-id="${escapeHtml(item.id)}" data-option="${index + 1}" ${item.selected_option ? "disabled" : ""}><span>${index + 1}. ${escapeHtml(option)}</span><strong>${Number(item.vote_counts && item.vote_counts[String(index + 1)]) || 0} Votes</strong></button>`).join("")}</div><p><strong>Total Votes: ${Number(item.total_votes) || 0}</strong>${item.selected_option ? `<br>✅ Your Vote: Option ${escapeHtml(item.selected_option)}` : ""}</p><small>One Legend • One Governance Vote. Legend Points are not deducted. This does not affect Worldz Votes Centre™ popularity.</small></article>`;
  }).join("") : empty("WorldzGovern™ has no active governance proposals right now.");
...
function renderRewards(rewards) {
  return rewards.length ? rewards.map((reward) => `<div class="reward"><span>⭐ ${escapeHtml(reward.reason || reward.reward_type || "Legend Reward")}</span><strong>${Number(reward.amount) > 0 ? "+" : ""}${Number(reward.amount) || 0}</strong></div>`).join("") : empty("No reward transactions yet.");
}
function renderCommunity() {
  ...
  byId("community-links").innerHTML = `<div class="links">${Object.entries(labels).filter(([key]) => links[key]).map(([key,label]) => `<a class="link" href="${escapeHtml(links[key])}" target="_blank" rel="noopener">${escapeHtml(label)}</a>`).join("")}</div><button class="button secondary" data-open="heroes">Open Real-World Heroes</button><button class="button secondary" data-open="kitty">Open Community Kitty</button><button class="button secondary" data-open="fullbuild">🌐 Open WorldzFullBuild™</button><button class="button secondary" data-open="fullscope">📡 Open WorldzFullScope™</button><button class="button secondary" data-open="memory-storage">🧹 Memory & Storage Centre</button><button class="button secondary" data-open="worldz-votes">🗳️ Popularity: Worldz Votes Centre™</button><button class="button secondary" data-open="governance">🏛️ Governance: WorldzGovern™</button>${state.data.admin ? '<button class="button secondary" data-open="admin">Open Admin Centre</button>' : ""}`;
}
...
function ensureOwnerTeamControls() {
  if (state.data.admin_access?.role !== "owner" || byId("admin-team")) return;
  const holder = document.createElement("div");
  holder.innerHTML = `<form id="admin-team" class="panel"><h3>Admin Team</h3><input name="telegram_id" inputmode="numeric" placeholder="Telegram ID" required><div class="form-row"><select name="role"><option>admin</option><option>moderator</option><option>recap_manager</option><option>partner_manager</option><option>treasury_manager</option></select><select name="status"><option>active</option><option>disabled</option></select></div><button class="button">Save Team Member</button></form><form id="admin-permission" class="panel"><h3>Permission Override</h3><input name="telegram_id" inputmode="numeric" placeholder="Telegram ID" required><input name="permission" placeholder="submission.approve" required><label class="toggle"><input name="enabled" type="checkbox" checked> Permission enabled</label><button class="button">Save Permission</button></form>`;
  byId("admin-panel").append(...holder.children);
}

document.addEventListener("submit", async (event) => {
  if (!["mission-create","points-rule","admin-team","admin-permission"].includes(event.target.id)) return;
  event.preventDefault();
  const form = new FormData(event.target);
  const payload = Object.fromEntries(form);
  try {
    if (event.target.id === "mission-create") { await api("/api/mini/admin/missions", { method:"POST", body:JSON.stringify(payload) }); feedback("✅ New Raaiiidd created."); }
    else if (event.target.id === "points-rule") { payload.enabled = form.has("enabled"); await api("/api/mini/owner/contribution-rules", { method:"POST", body:JSON.stringify(payload) }); feedback(`✅ ${payload.asset} contribution rule saved.`); }
    else if (event.target.id === "admin-team") { await api("/api/mini/owner/admins", { method:"POST", body:JSON.stringify(payload) }); feedback("✅ Admin Team record saved."); }
    else { payload.enabled = form.has("enabled"); await api("/api/mini/owner/permissions", { method:"POST", body:JSON.stringify(payload) }); feedback("✅ Permission override saved."); }
    await renderAdmin(); setTimeout(ensureOwnerTeamControls, 0);
  } catch (error) { feedback(`Update failed: ${error.message}`); }
});

document.addEventListener("click", async (event) => {
  const vote = event.target.closest(".governance-vote");
  if (vote) { try { await api(`/api/mini/governance/${vote.dataset.proposalId}/vote`, { method:"POST", body:JSON.stringify({ selected_option:vote.dataset.option }) }); feedback("✅ WorldzGovern™ vote recorded. This does not affect token popularity rankings."); state.data = await api("/api/mini/bootstrap"); render(); showScreen("governance"); } catch (error) { feedback(`Vote not recorded: ${error.message}`); } return; }
