...
document.addEventListener('click',async(event)=>{const copy=event.target.closest('.copy-address');if(copy){try{await navigator.clipboard.writeText(copy.dataset.address);const original=copy.textContent;copy.textContent=`✅ ${copy.dataset.asset} Address Copied`;setTimeout(()=>{copy.textContent=original;},1800);if(tg&&tg.HapticFeedback)tg.HapticFeedback.notificationOccurred('success');}catch{copy.textContent='Press and hold the address to copy';}return;}const target=event.target.closest('[data-screen],[data-open]');if(target)showScreen(target.dataset.screen||target.dataset.open);});
